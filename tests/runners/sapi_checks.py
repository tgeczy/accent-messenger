"""Test existing SAPI builds using isolated data, settings and in-memory audio."""
import argparse
import hashlib
...
def main():
    ...
    parser.add_argument('--scratch',type=Path,help='New directory for staged test files')
    parser.add_argument('--output',type=Path,help='JSON report path')
    args=parser.parse_args()
    stage=args.scratch or Path(tempfile.mkdtemp(prefix='Messenger SAPI test '))/'stage'
    stage.mkdir(parents=True,exist_ok=False)
    (stage/'data').mkdir()
    shutil.copy2(ROOT/'assets/SPKMIC.TSR',stage/'data/SPKMIC.TSR')
    ...
    fixture=stage/'number-parity.tsv'
    ...
    for arch in ('x86','x64'):
        ...
        settings=stage/('sapi-test-'+arch+'.toml')
        ...
        dialog=subprocess.run([str(build/'sapi/MinSizeRel/settings_test.exe'),str(stage/('sapi-gui-test-'+arch+'.toml'))],capture_output=True,text=True,timeout=20)
    ...
    output=args.output or ROOT/'dist/sapi-validation.json'
    output.parent.mkdir(parents=True,exist_ok=True)
    output.write_text(json.dumps({'architectures':reports,
