...
# Reused from Tamas Geczy's outspoken-nvda (MIT); license accompanies this
# module. Messenger always selects English. Local change: strip leading zeros
# before int(), so arbitrarily long zero-padded input cannot abort speech.
    ...
    def sub(mo):
        ...
        if mo.group("ord"):
            ...
            if big(head):
                return large
            if lang == "es":
                return cardinal(int(head.lstrip('0') or '0'), lang)
            return ordinal(int(head.lstrip('0') or '0'))
        ...
        if spell_out:
            ...
            out = cardinal(int(whole.lstrip('0') or '0'), lang)
        if frac:
